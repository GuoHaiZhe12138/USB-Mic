...
__ALIGN_BEGIN static uint8_t USBD_AUDIO_CfgDesc[] __ALIGN_END =
{
    ...
    0x90, 0x00,                     // wMaxPacketSize = 192 bytes (48kHz x 2 bytes x 1 ch / 1ms)
...
static uint8_t  USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  {
    /* ���ͻ��������ݵ����� */
    ((USBD_AUDIO_ItfTypeDef *)pdev->pUserData[0])->AudioCmd(haudio->buffer,
                                                         AUDIO_IN_PACKET,
                                                         AUDIO_CMD_START);
...
static void AUDIO_REQ_GetCurrent(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
    uint8_t response[2] = {0x00, 0x00}; // Ĭ�Ͼ����ر�
